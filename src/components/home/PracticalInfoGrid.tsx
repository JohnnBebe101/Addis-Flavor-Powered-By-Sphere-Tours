...
export const PracticalInfoGrid: React.FC<PracticalInfoGridProps> = ({
  ...
  return (
    <section id="practical-info" className="py-12 bg-sandstone/10 border-b border-teal/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {columns.map((column) => (
            <div
              key={column.id}
              className="p-6 rounded-2xl border border-teal/10 bg-linen-white hover:border-gold/40 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
            >
              <div className="w-12 h-12 rounded-xl bg-gold/10 text-gold flex items-center justify-center mb-4">
                {column.icon}
              </div>
              <h3 className="text-lg font-serif font-bold text-teal mb-2">{column.title}</h3>
              <p className="text-sm text-teal/70 leading-relaxed">{column.description}</p>
            </div>
          ))}
        </div>
