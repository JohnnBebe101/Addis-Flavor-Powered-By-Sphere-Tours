...
export const HowBookingWorks: React.FC<HowBookingWorksProps> = ({ steps, headline }) => {
  return (
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="relative text-center">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-lg font-serif font-bold text-teal mb-2">{step.title}</h3>
                <p className="text-sm text-teal/70 leading-relaxed">{step.description}</p>
              </div>
            </div>
          ))}
        </div>
